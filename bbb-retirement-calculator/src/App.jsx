...
function calculate(inputs) {
  ...
  for (let age = currentAge; age <= retirementAge; age += Math.max(1, Math.floor(n / 10))) {
    ...
    const target = yr === 0 ? 0 : totalCorpusNeeded * (yr / n);
...
function TimelineChart({ data }) {
  ...
  const pathD = (accessor, smooth = false) => {
    return data.map((d, i) => {
...
export default function RetirementCalculator() {
  ...
  const set = (key) => (val) => {
    setInputs(prev => {
      ...
      // Auto-update employer EPF rate when salary changes
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: "#f8fafc", minHeight: "100vh" }}>
      <style>{`
        ...
        input[type=range] { -webkit-appearance: none; appearance: none; background: #d9e2ec; border-radius: 4px; }
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 16px; height: 16px; border-radius: 50%; background: #c9a84c; cursor: pointer; border: 2px solid #fff; box-shadow: 0 2px 6px rgba(0,0,0,0.2); }
      ...
      <header style={{
        ...
        padding: "0 24px", boxShadow: "0 4px 20px rgba(0,0,0,0.3)"
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, color: "#0a192f", fontSize: 14 }}>BBB</div>
          <div>
            <div style={{ fontWeight: 800, color: "#fff", fontSize: 14, letterSpacing: "0.05em" }}>BBB WEALTH</div>
            <div style={{ fontSize: 9, color: "#829ab1", textTransform: "uppercase", letterSpacing: "0.12em" }}>Retirement Planning Calculator</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
          <a href="https://wa.me/601156281541" target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, color: "#9fb3c8", textDecoration: "none", fontSize: 13, fontWeight: 600, transition: "color 0.2s" }}>
            <Icons.MessageCircle size={15} />WhatsApp
          </a>
          <a href="https://www.bbbwealth.com" target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, color: "#9fb3c8", textDecoration: "none", fontSize: 13, fontWeight: 600 }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 16px 60px" }}>
        ...
        <div style={{ borderRadius: 20, overflow: "hidden", boxShadow: "0 16px 48px rgba(10,25,47,0.14)", border: "1px solid #d9e2ec" }}>
          ...
          <div style={{ background: "#fff", padding: "28px 28px" }}>
            ...
            {/* EPF rate display bar */}
            <div style={{ background: "linear-gradient(135deg, #0a192f, #1a365d)", borderRadius: 12, padding: "14px 20px", display: "flex", alignItems: "center", gap: 12, marginBottom: 32 }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <button
                ...
                onMouseLeave={e => { e.target.style.transform = ""; e.target.style.boxShadow = "0 4px 20px rgba(201,168,76,0.35)"; }}>
        ...
        {/* ── RESULTS ── */}
        {showResults && result && (
          <div ref={resultsRef} className="fade-in" style={{ marginTop: 40, display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#fdf9ed", border: "1px solid #f3e0a3", borderRadius: 20, padding: "8px 20px" }}>
                <span className="bounce-arrow" style={{ display: "inline-block" }}><Icons.ArrowDown size={14} style={{ color: "#c9a84c" }} /></span>
                ...
                {[
                  {
                    ...
                    title: "Boost Income",
                    desc: "Upgrade your skills for promotions and raises, or build side income streams to accelerate your EPF contributions.",
                    btn: "Adjust Salary Growth",
                    onClick: () => {
                      salaryGrowthRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
                      setTimeout(() => salaryGrowthRef.current?.focus(), 400);
                    }
                  },
                  {
                    icon: Icons.CircleDollarSign,
                    title: "Disciplined Investing",
                    desc: "Build a diversified global portfolio (70% Equities + 30% Fixed Income) to earn higher returns than EPF alone.",
                    info: "The gap signals it's time to invest beyond EPF. Review your Investment Return Rate in the assumptions above.",
                  },
                  {
                    icon: Icons.Clock,
                    title: "Delay Retirement",
                    desc: "Working a few more years gives your EPF and investments extra time to compound, reducing the gap significantly.",
                    btn: "Adjust Retirement Age",
                    onClick: () => {
                      retirementAgeRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
                      setTimeout(() => retirementAgeRef.current?.focus(), 400);
                    }
                  }
                  ...
                  <div key={title} style={{ background: "linear-gradient(135deg, #0a192f, #112240)", borderRadius: 16, padding: "20px 18px" }}>
                    ...
                    {btn && (
                      <button onClick={onClick} style={{ width: "100%", padding: "10px 0", borderRadius: 8, border: "1px solid #c9a84c", background: "transparent", color: "#c9a84c", fontWeight: 700, fontSize: 12, cursor: "pointer", transition: "background 0.15s" }}
            ...
            <Card style={{ background: "linear-gradient(135deg, #fdf9ed, #fff)", borderTop: "4px solid #c9a84c" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 16 }}>
                {[
                  ...
                  <div key={n} style={{ background: "#fff", borderRadius: 14, padding: "18px 16px", boxShadow: "0 4px 12px rgba(10,25,47,0.06)", transition: "transform 0.15s" }}
        ...
        <div style={{ marginTop: 48, background: "linear-gradient(135deg, #0a192f, #112240)", borderRadius: 20, padding: "28px 28px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 24, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <a href="https://www.bbbwealth.com" target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, color: "#c9a84c", textDecoration: "none", fontWeight: 700, fontSize: 14 }}>
                <Icons.Globe size={16} /> www.bbbwealth.com <Icons.ArrowRight size={14} />
              </a>
            </div>
          </div>
        </div>
